...
│   └── tools.py                    # Agentic tool framework
│
├── 📄 Document Processing
...
│   ├── self_rag.py                 # Alternative Self-RAG implementation
│   └── config_param.py             # Prompts and configuration
│
├── 🕸️ GraphRAG Module
...
│   ├── graph_builder.py            # Graph construction pipeline
│   ├── graph_retriever.py          # Hybrid vector + graph retrieval
│   └── graphrag_inferencer.py      # GraphRAG inference engine
│
├── 📈 Evaluation
│   ├── rag_eval.py                 # RagEvaluator with metrics
│   ├── eval_dataset_prep_ground_truth.py
│   └── rag_evaluation_data_prep.py
│
└── 🏃 Runners
...
#### Self-RAG Pipeline (with Critique Loop)
...
└─────────────┘     └─────────────────┘     └────────┬────────┘
              ...
              │  ┌─────────────────────────────────────┐   │
              │  │  GenAIChatClient                    │   │
              │  │  "Generate answer using context"    │───┼──▶ draft_answer
              │  └─────────────────────────────────────┘   │
              └─────────────────────────────────────────────┘
                                    │
                                    ▼
              ...
              └─────────────────────────────────────────────┘
                                    ...
                                    ▼
              ┌─────────────────────────────────────────────┐
              │  DataFrame: [question, draft, critique,    │
              │              final_answer]                  │
              └─────────────────────────────────────────────┘
```

#### GraphRAG Pipeline (Full Architecture)
...
│                  │  │ • works_for, manages, created_by            │   │   │
│                  │  │ • located_in, part_of, owns, uses           │   │   │
│                  │  │ • founded, acquired, invested_in            │   │   │
│                  │  └─────────────────────────────────────────────┘   │   │
│                  └──────────────────────┬──────────────────────────────┘   │
│                                         ▼                                   │
│                  ┌─────────────────────────────────────────────────────┐   │
...
│                  │  │  Dict[id,Entity]│  │  List[Relationship]     │  │   │
│                  │  └────────┬────────┘  └───────────┬─────────────┘  │   │
│                  │           │         ┌─────────────┘               │   │
│                  │           ▼         ▼                             │   │
│                  │  ┌───────────────────────────────────────────┐   │   │
│                  │  │ Adjacency Indices                         │   │   │
│                  │  │ • _adjacency_out: entity → outgoing rels  │   │   │
│                  │  │ • _adjacency_in: entity → incoming rels   │   │   │
│                  │  │ • _name_to_id: fast name lookup           │   │   │
│                  │  │ • _type_index: entities by type           │   │   │
│                  │  │ • _chunk_to_entities: chunk references    │   │   │
│                  │  └───────────────────────────────────────────┘   │   │
│                  └─────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
│       ▼                         ▼                           ▼              │
│  ┌─────────────┐         ┌─────────────────┐        ┌──────────────────┐  │
│  │   Vector    │         │    Graph        │        │   Name Match     │  │
│  │   Search    │         │   Retriever     │        │   (fuzzy)        │  │
│  │             │         │                 │        │                  │  │
│  │ FAISS/Azure │         │ • Entity embed  │        │ • find_entities  │  │
│  │ top_k × 2   │         │   similarity    │        │   _by_name()     │  │
│  └──────┬──────┘         │ • Graph expand  │        └────────┬─────────┘  │
│         │                │   (multi-hop)   │                 │            │
│         │                └────────┬────────┘                 │            │
│         │                         │                          │            │
...
└────────┘ └────────┘       └────────┘ └────────────┘    └────────┘ └────────────┘
   ...
   └──────────────┘
    ...
    │                     │       │                     │
    │ • _run_draft()      │       │ • build_graph()     │
    │ • _run_critique()   │       │ • _retrieve_with_   │
    │ • fact-checking     │       │   graph()           │
    └─────────────────────┘       │ • query_graph()     │
                                  └─────────────────────┘
    ...
    │ • entity_type   │       │ • relation_type │       │ • parameters    │
...
│  │  • Azure Search │  │  • Relations    │  │  • Metadata                 │  │
│  │  • Milvus       │  │  • Adjacency    │  │                             │  │
│  └─────────────────┘  └─────────────────┘  └─────────────────────────────┘  │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
...
│  │  │ • Extract QA pairs from documents using LLM                 │ │     │
...
│  │  │  │ • BLEU Score     │  │ • Answer Relevance (0.0-1.0)     ││ │     │
│  │  │  │ • Precision      │  │ • Accuracy (0.0-1.0)             ││ │     │
│  │  │  │ • Recall         │  │ • Completeness (0.0-1.0)         ││ │     │
│  │  │  │ • F1 Score       │  │ • Clarity (0.0-1.0)              ││ │     │
