...
class RagEvaluator:
    ...
    def _extract_llm_scores(self, response_text):
        ...
        try:
            scores["answer_relevance"] = float(
                re.search(
                    r"Answer Relevance:\s*([0-1](?:\.\d+)?)", response_text
                ).group(1)
            )
            scores["accuracy"] = float(
                re.search(r"Accuracy:\s*([0-1](?:\.\d+)?)", response_text).group(1)
            )
            scores["completeness"] = float(
                re.search(r"Completeness:\s*([0-1](?:\.\d+)?)", response_text).group(1)
            )
            scores["clarity"] = float(
                re.search(r"Clarity:\s*([0-1](?:\.\d+)?)", response_text).group(1)
            )
            scores["semantic_similarity"] = float(
                re.search(
                    r"Semantic Similarity Score:\s*(\d{1,3})%", response_text
                ).group(1)
            )
    ...
    def evaluate(
        self, input_df: pd.DataFrame, prompt: str = None, max_rows: int = None
    ):
        if max_rows:
        ...
        for idx, row in input_df.iterrows():
            ...
            bleu = sentence_bleu(
                reference, candidate, smoothing_function=self.smoothing_function
            )
            ...
            binary_pred.append(
                1
                if scores["semantic_similarity"] and scores["semantic_similarity"] >= 80
                else 0
            )
        ...
        with mlflow.start_run():
            ...
            mlflow.log_params(
                {"evaluation_size": len(input_df), "total_questions": len(input_df)}
            )
